#!/bin/sh
#-------------------------------------------------------------
# Oracle Database 11g audit script - OS side
# Copyright (c) Deloitte Touche Tohmatsu India LLP (c) 2019
#-------------------------------------------------------------
#
...
#
#---------------------------------------------------------------
# Upon completion, a tarball, called Deloitte_<uname -n>.tar 
# will be created under /tmp/Deloitte_oracle
#---------------------------------------------------------------

VERSION=1.0
ORACLE_PATH=$ORACLE_HOME/../../..
TMP_PATH=/tmp/Deloitte_oracle
...
echo "if no tmppath is specified, \"/tmp/Deloitte_oracle\" will be assumed" ; echo
}
...
echo "8. Obtaining listener information"
$ORACLE_HOME/bin/lsnrctl status > $TMP_PATH/OSlistener_status.txt
$ORACLE_HOME/bin/lsnrctl show log_status > $TMP_PATH/OSlistener_log.txt
echo "DONE." ; echo

echo "9. Adding output files to tarball under $TMP_PATH"
tar cvf "$TMP_PATH/Deloitte_`uname -n`.tar" $TMP_PATH/*.txt \
$TMP_PATH/oracle_home_dbs.tar --exclude $TMP_PATH/examine.txt \
2&> /dev/null
echo "DONE." ; echo

echo "2.7.5 DBMS software libraries should be periodically backed up."
echo "Manual Check"
echo "DONE." ; echo
